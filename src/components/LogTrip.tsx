            ...
            {prioritizedDrivers.map((driver, index) => {
              ...
              if (isSelected) {
                return (
                  <div
                    key={driver.id}
                    className="w-full p-5 rounded-2xl border-2 border-national-yellow bg-white/10 shadow-[0_0_30px_rgba(255,204,0,0.1)] text-left transition-all relative overflow-hidden"
                  >
                    {/* Header of Selected Driver */}
                    <div className="flex justify-between items-start mb-4 pb-3 border-b border-white/10">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-lg flex items-center justify-center font-black text-[11px] bg-national-yellow text-national-green">
                          {index + 1}
                        </div>
                        <div>
                          <p className="font-black text-base uppercase tracking-tight text-white">{driver.firstName} {driver.lastName}</p>
                          <div className="flex flex-wrap items-center gap-1.5 mt-0.5">
                            <span className="text-[8px] font-black text-white/30 uppercase tracking-widest">Servicios</span>
                            <span className="text-xs font-black font-mono text-national-yellow mr-2">{driver.totalPoints}</span>
                            {driver.billingStatus === 'delayed' ? (
                              <span className="text-[8px] bg-amber-500/20 text-amber-300 font-extrabold px-1 py-0.5 rounded uppercase tracking-wider">
                                ⚠️ RETRASO EN TICKETS
                              </span>
                            ) : (
                              <span className="text-[8px] bg-green-500/20 text-green-300 font-extrabold px-1 py-0.5 rounded uppercase tracking-wider">
                                ✓ AL CORRIENTE
                              </span>
                            )}
                          </div>
                      ...
                      <button
                        ...
                        className="text-[9px] font-black uppercase tracking-wider px-2.5 py-1 rounded bg-white/5 hover:bg-white/15 text-white/60 hover:text-white transition-colors cursor-pointer"
                      >
                        Cambiar
                      </button>
                    </div>

                    {/* Integrated Options & Input Form */}
                    <div className="space-y-4">
                      <div>
                        <label className="block text-[9px] font-black text-white/40 uppercase tracking-widest mb-1.5">Respuesta de {driver.firstName}</label>
                        <div className="grid grid-cols-2 bg-black/20 p-1 rounded-xl gap-2 border border-white/5">
                          <button
                            type="button"
                      ...
                      {tripStatus === 'completed' ? (
                        <div className="space-y-2">
                          <label className="block text-[9px] font-black text-white/40 uppercase tracking-widest">Dinero para Comida ($ MXN)</label>
                          <div className="relative">
                            <span className="absolute left-3 top-1/2 -translate-y-1/2 font-black text-white/40 font-mono text-xs">$</span>
                            <input
                              type="number"
                              placeholder="Ej. 500"
                              value={foodAllowance}
                              ...
                              className="w-full bg-black/20 border border-white/10 rounded-xl pl-7 pr-3 py-2.5 focus:outline-none focus:ring-1 focus:ring-national-yellow font-mono font-bold text-white text-xs placeholder:text-white/20"
                            />
                          </div>
                          <p className="text-[9px] text-white/30">Monto entregado al colaborador para viáticos de comida.</p>
                        </div>
                      ) : (
                        <div className="space-y-3">
                          <div>
                            <label className="block text-[9px] font-black text-white/40 uppercase tracking-widest mb-1.5">Razón de Negación</label>
                            <select
                              value={deniedReason}
                              onChange={(e) => setDeniedReason(e.target.value as any)}
                              className="w-full bg-[#1e1e1e] border border-white/10 rounded-xl px-3 py-2.5 focus:outline-none focus:ring-1 focus:ring-national-yellow font-bold text-white text-[11px] cursor-pointer"
                            >
                              <option value="no quiso">No quiso realizar el traslado</option>
                              <option value="esta en turno">Está en turno operativo</option>
                              <option value="tiene chofereada">Tiene chofereada activa</option>
                              <option value="sin opcion por faltas">Sin opción por faltas</option>
                            </select>
                          </div>

                          <div>
                            <label className="block text-[9px] font-black text-white/40 uppercase tracking-widest mb-1.5">Tipo de Traslado Ofrecido</label>
                            <div className="grid grid-cols-2 bg-black/20 p-1 rounded-xl gap-2 border border-white/5">
                              <button
                                type="button"
                      ...
                      {/* Immediate Core Action Button inside the card */}
                      <div className="pt-2">
                        <button
                          type="submit"
                          disabled={submitting}
                          className={`w-full font-black py-4 px-6 rounded-xl shadow-lg transition-all uppercase tracking-wider text-[11px] flex items-center justify-center gap-2.5 disabled:opacity-30 active:scale-95 transition-all cursor-pointer ${
                            tripStatus === 'denied'
                              ? 'bg-red-500 text-white hover:bg-red-600 shadow-red-950/20'
                              : 'bg-national-yellow text-national-green hover:bg-yellow-400 shadow-national-yellow/20'
                          }`}
                        >
              ...
              return (
                <button
                  ...
                  onClick={() => {
                    ...
                    setTripStatus('completed'); // Default back to accepted on swap
                  }}
                  className="w-full py-4 px-5 rounded-xl border-2 border-white/5 bg-white/5 hover:border-white/10 text-left transition-all relative overflow-hidden group cursor-pointer"
                >
                  <div className="flex justify-between items-center relative z-10">
                    <div className="flex items-center gap-4">
                      <div className="w-10 h-10 rounded-xl flex items-center justify-center font-black text-xs bg-white/10 text-white/40">
                        {index + 1}
                      </div>
                      <div>
                        <p className="font-black text-lg uppercase tracking-tight text-white">{driver.firstName} {driver.lastName}</p>
